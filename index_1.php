...
<body>
    ...
    class Users {
        ...
        public function fetchData($date, $connection){
            $this->sql = "SELECT user_id, name, address, phone, email, password, SUM(price), currency FROM users JOIN transactions ON users.id=transactions.user_id WHERE transactions.created_at >= '$date' GROUP BY user_id, currency LIMIT 20";
            $this->result = $connection->query($this->sql);
